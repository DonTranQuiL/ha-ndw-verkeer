...
def test_ndw_diagnostic_sensor_status(mock_coordinator, mock_entry):
    ...
    sensor = NDWDiagnosticSensor(
        mock_coordinator, mock_entry, "last_update_status", "Last Update Status"
    )
...
def test_ndw_diagnostic_sensor_time_and_errors(mock_coordinator, mock_entry):
    ...
    time_sensor = NDWDiagnosticSensor(
        mock_coordinator, mock_entry, "last_update_time", "Last Update Time"
    )
    assert time_sensor.native_value == "2026-05-19T11:49:00Z"

    mock_coordinator.error_count = 5
    error_sensor = NDWDiagnosticSensor(
        mock_coordinator, mock_entry, "error_count", "Consecutive Errors"
    )
